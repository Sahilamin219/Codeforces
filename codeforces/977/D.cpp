...
void solve(){
  ...
  while(v.size()){
    ...
    else if(g==dq.back()/3 and dq.back()%3==0){
      dq.push_back(g);
      v.erase(g);
      m[g]++;
      continue;
    }
    else if(g%3==0 and m.find(g/3)!=m.end()){
      dq.push_front(g);
      v.erase(g);
      m[g]++;
      continue;
    }
    else if(m.find(2*g)!=m.end()){
      dq.push_front(g);
      v.erase(g);
      m[g]++;
      continue;
  ...
  for(int i=n-1;i>=0;i--){
    for(int j=i;j<n;j++){
      if(i==j)DP[i][j]=0;
      else DP[i][j]=DP[i+1][j-1] + (a[j]-a[i])*(b[i] - b[j]);
...
public:
  ...
  int solveit(){
    ...
    // We remove all the smaller results greedily as they wont get used anyways 
    // (This helps the monoqueue to remain sorted)
    ...
    // Erase all the indices in deque that are greater than or equal to i+k.
        if(dq.front()>=i+k)
            dq.pop_front();
        nums[i]=curr;
    // Use input array as auxillary array to store the best results.
    }
    return curr;
  }
  // `curr` would have the best or the maxium result to reach end from index 0.
