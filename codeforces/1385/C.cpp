...
int32_t main()
{
    ...
    while(t--){
        ...
        ll l=n-1;
        ll mi=100001;
        while(l>0 and v[l]<=v[l-1]){
            mi=min(mi, v[l]);
            l--;
        }
        int s=l;
        while(s>0 and v[s]>=v[s-1])
       {
            mi=min(mi, v[s]);
            s--;
        }
        // r(i,l,1)
        // {
        //     if(v[i]<v[i-1]){ans=i;break;}
        //     if(v[i]<=mi and v[i]!=v[i+1]){ans=i-1;break;}
        //     mi=min(mi, v[i]);
        // }
        // if(*min_element(all(v))== *max_element(all(v)))ans=0;
        // if(ans==1 and n>1)ans=2;
        cout<<s<<endl;
    }
