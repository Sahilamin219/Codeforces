...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<m;j++){
            if(v[i][j]=='G' and flag==false){
                flag=true;
                gap=1;
            }
            else if(v[i][j]=='G' and flag){
                InBetween.push_back(gap-1);
            }
            else if(v[i][j]=='*' and flag)gap++;
            else if(v[i][j]=='S' and flag){
                res.push_back(gap);
                while(InBetween.size())
                {
                    res.push_back(gap - InBetween.back());
                    InBetween.pop_back();
                }
                gap=0;
                flag=false;
