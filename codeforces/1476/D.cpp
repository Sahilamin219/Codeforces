...
void solve_me()
{
  ...
  int cities_accumulated = 0;
  int last_city_visited = -1;
  for(i=0;i<n;i++)
  {
    if( last_city_visited != 1 and s[i]=='L')
    {
      cities_accumulated++;
    }
    else if(last_city_visited !=0 and s[i]=='R')
    {
      cities_accumulated++;
    }
    else
    {
      cities_accumulated=1;
    }
    last_city_visited=(s[i] == 'L');
    if(s[i] == 'L')
    {
      ans[i+1] += cities_accumulated;
    }
  }
  last_city_visited=-1;
  cities_accumulated=0;
  for(i=n-1;i>=0;i--)
  {
    if( last_city_visited != 1 and s[i]=='L')
    {
