...
void solve(){ 
	...
	for(auto &i:v)cin>>i;
	vector<ll> prcie(5);
	for(ll i=0;i<5;i++)cin>>prcie[i];
	vector<ll> res(5);
	ll amount=0;
	for(ll i=0;i<n;i++){
		amount += v[i];
		skip:;
		ll j=lower_bound(all(prcie),amount)-prcie.begin();
		// cout<<amount<<" "<<prcie[j]<<"\n";
		ll q=0;
		if( j == 0 and prcie[j]>amount)continue;
		else if(amount!=prcie[j])j--;
		// cout<<j<<"\n";

		q=amount/prcie[j];
		res[j]+=q;
		amount=amount%prcie[j];
		goto skip;
	}
