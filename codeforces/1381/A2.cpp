...
int32_t main()
{
	...
	{
		cin>>n>>a>>b;
		vector<int> operation_1, operation_2;
		a+="0";
		b+="0";
		f(i,1,n+1)
		{
			if(a[i]!=a[i-1])operation_1.pb(i);
			if(b[i]!=b[i-1])operation_2.pb(i);
		}
		operation_1.insert(operation_1.end(), operation_2.rbegin(), operation_2.rend());
		cout<<operation_1.size()<<" ";
		for(auto x:operation_1){cout<<x<<" ";}
		cout<<endl;
	}
	// ll n,m;cin>>n>>m;
