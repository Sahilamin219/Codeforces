...
ll k,a,b;
void recursion(string s, int index, vector<string> res, int frequent)
{
	if(index<=s.size() and index+b >= s.size() and frequent==k)
	{
		for(auto x: res)cout<<x<<"\n";
		for(int i=index;i<s.size();i++)cout<<s[i];
		exit(0);
	}
	else if(index<s.size() and frequent <= k)
	{
		for(int i=index;i<s.size();i++)
		{
			if(i-index+1 >= a and i-index+1 <= b)
			{
				res.push_back(s.substr(index, i-index+1));
				recursion(s, i+1, res, frequent+1);
				res.pop_back();
			}
		}
	}
...
void solve()
{
	...
	// recursion(s, 0, res, 1);
	ll n=s.size();
	if(n > b*k or n< a*k)cout<<"No solution";
	else{
		for(int i=0;i<k;i++)
		{
