...
*/
string s;
int solve(string s, int n, int a, int start, int end)
{
	int temp=0;
	f(i,start,end/2)
	{
...
int32_t main()
{
	...
	while(t--)
	{
		...
		ll total_parity=0;
		vector<ll> total_bits(32,0);
		f(i,0,n)
		{
			total_parity=total_parity^v[i];
		}
		// cout<<total_parity<<"--";
		if(total_parity==0){cout<<"DRAW\n";}
		else
		{
			f(i,0,n)
			{
				f(j,0,32)
				{
					if((v[i]>>j)&1)total_bits[j]++;
					// total_bits[j]=total_bits[j]+(v[i]&(1<<j));
					// cout<<(v[i]&(1<<j))<<"  ";
				}//cout<<"\n";
			}
			int flag=1;
